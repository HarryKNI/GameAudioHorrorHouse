    ...
    public class DemonScript : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (Time.time >= LastBehoviourTime + BehoviourPeriod)
            {
                ...
                {
                    if (!hasRealizedPlayer)
                    {
                        if (IsPlayerVisible() && !HeroPlayerScript.Instance.isHiding)
                        {
                            target = HeroPlayerScript.Instance.transform;
                            hasRealizedPlayer = true;
                        }
                ...
                if (target == null && currentStatus == BehoviourType.Idle)
                {
                    if (Time.time >= LastIdleTime + IdlePeriod)
                    {
                        ...
                        if (decision == 0 || PointsToPatrol.Count == 0)
                        {
                        ...
                        {
                            targetLocation = PointsToPatrol[UnityEngine.Random.Range(0, PointsToPatrol.Count)];
        ...
        {
            Vector3 direction = (HeroPlayerScript.Instance.transform.position - transform.position).normalized;
            ...
            if (Physics.Raycast(transform.position, direction, out hit, distance))
            {
                ...
                if (hit.collider.transform == HeroPlayerScript.Instance.transform)
                    return true;
        ...
        public void GetDamageByCross(float damage)
        {
            if (health <= 0) return;
